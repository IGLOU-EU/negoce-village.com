...
update_site() {
    ...
    local file="$1"
    local css='<style type="text/css">#mainNav.type1 ul li ul.submenu {left: auto !important}body{overflow: auto !important}#s4-workspace{width: auto !important;height: auto !important}</style><style type="text/css" media="screen and (max-width: 767px)" rel="stylesheet" href="'"$CSS_FNA_MOBILE_I"'"></style><style type="text/css" media="screen and (max-width: 767px)" rel="stylesheet" href="'"$CSS_FNA_MOBILE_II"'"></style>'
    ...
    if ! sed -i 's|</head>|'"$css"'</head>|g' "$file"; then
        error "Failed to add css in '$file'"
    fi
