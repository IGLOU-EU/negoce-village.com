...
fix_error() {
    ...
    # we use file --mime-type 
    case $(file --mime-type "$file" | sed 's/.*: //') in
        text/html | application/xml | text/asp)
